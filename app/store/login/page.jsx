...
export default function StoreLoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      console.log('[StoreLogin] Attempting login with email:', email)
      ...
      console.log('[StoreLogin] Response status:', res.status)
      
      if (!res.ok) {
        console.error('[StoreLogin] Login failed:', data?.error)
      ...
      if (data.token) {
        ...
        console.log('[StoreLogin] Token stored in localStorage')
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold mb-2 text-slate-800">Store Login</h1>
          <p className="text-slate-600 mb-8">Login to manage your products</p>
          ...
          <p className="text-center text-slate-600 text-sm mt-6">
            Use your store admin credentials
          </p>
        </div>
      </div>
    </div>
